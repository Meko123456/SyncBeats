...
class AndroidGoogleAuthController(private val context: Context) : GoogleAuthController {
     ...
     *
     * Looked up reflectively on purpose. Referencing `R.string.default_web_client_id` directly
     * would not compile on a checkout whose google-services.json has no OAuth client — which is
     * the same reason the plugin itself is applied conditionally — so the choice is between this
     * and a build that only works for one person.
     *
     * The cost is that resource shrinking cannot see the reference and would strip the string,
     * breaking Google sign-in in release builds only. `res/raw/keep.xml` prevents that.
     */
    @Suppress("DiscouragedApi") // see above: the alternative is a build that will not compile
